...
export function AwsConnection({ onComplete, externalId }: AwsConnectionProps) {
	...
	const cfnUrl =
		"https://console.aws.amazon.com/cloudformation/home#/stacks/create/template?stackName=Grape-Onboarding";
	...
	const onSubmit = async (data: AwsRoleFormValues) => {
		...
		try {
			...
			console.error(error);
			toast.error(
				"Failed to connect account. Please check the role ARN.",
			);
		...
		<div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				<div className="md:col-span-1 space-y-4">
					...
					>
						<div
							className={`p-2 rounded-lg w-fit mb-3 ${method === "cloudformation" ? "bg-cyan-500 text-white" : "bg-slate-100 text-slate-500"}`}
						>
					...
					>
						<div
							className={`p-2 rounded-lg w-fit mb-3 ${method === "terraform" ? "bg-purple-500 text-white" : "bg-slate-100 text-slate-500"}`}
						>
					...
					<CardContent className="space-y-6">
						{method === "cloudformation" ? (
							<div className="space-y-6">
								<div className="flex gap-4">
									<div className="shrink-0 w-8 h-8 rounded-full bg-cyan-100 text-cyan-700 flex items-center justify-center font-bold">
										1
									</div>
									<div>
										...
										<p className="text-sm text-slate-500 mt-1 mb-3">
											Download the template, then upload
											it to the CloudFormation console
										...
										<div className="flex gap-3">
											...
											<Button
												...
												className="bg-linear-to-r from-cyan-600 to-cyan-500 hover:from-cyan-700 hover:to-cyan-600 shadow-md"
												type="button"
											>
							...
							<div className="space-y-6">
								<div className="flex gap-4">
									<div className="shrink-0 w-8 h-8 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center font-bold">
										1
									</div>
								...
								<div className="flex gap-4">
									...
									<div className="flex-1">
										...
										<p className="text-sm text-slate-500 mt-1">
											...
											and copy the output <b>role_arn</b>.
										</p>
									</div>
								</div>
							</div>
						...
						<div className="pt-6 border-t border-slate-100">
							...
							<div className="mt-4 flex items-start gap-2 p-3 bg-blue-50 rounded-lg border border-blue-100 text-blue-800 text-xs">
								...
								<p>
									Security Check: Grape will verify it can
									assume this role using the unique External
									ID provided. This prevents unauthorized
